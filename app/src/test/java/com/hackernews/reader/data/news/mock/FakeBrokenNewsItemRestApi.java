...
public class FakeBrokenNewsItemRestApi implements RestApiInterface {

    private RequestQueue requestQueue;
    private boolean enableException;

    public FakeBrokenNewsItemRestApi(boolean enableException){
    ...
    public RequestQueue getRequestQueue() {
        if(requestQueue == null){
            requestQueue = new FakeBrokenNewsItemRequestQueue(enableException);
            requestQueue.start();
        }
    ...
    private static class FakeBrokenNewsItemRequestQueue extends RequestQueue {
        public FakeBrokenNewsItemRequestQueue(boolean enableException) {
            super(new NoCache(),
                    new BasicNetwork(new FakeBrokenNewsItemHttpStack(enableException)),
                    1,
                    new ExecutorDelivery(Executors.newSingleThreadExecutor()));
        }
    }

    private static class FakeBrokenNewsItemHttpStack implements HttpStack {

        private boolean enableException = false;

        FakeBrokenNewsItemHttpStack(boolean enableException) {
            this.enableException = enableException;
        }
